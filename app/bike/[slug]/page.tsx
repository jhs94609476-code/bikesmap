...
// 빌드 부하 방지: 상위 30개만 SSG, 나머지는 첫 요청 시 렌더링
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${station.seoTitle} | BikesMap`,
...
const FAQ = [
  {
    q: '자전거는 어떻게 대여하나요?',
    a: '대여소 거치대에서 자전거를 확인 후 무인 단말기 또는 앱을 통해 신청합니다. 신용카드·교통카드 결제가 가능한 곳이 많습니다.',
  },
  {
    q: '반납은 어디서 하나요?',
    a: '지정된 거치대에 반납 후 잠금을 확인하세요. 동일 권역 내 다른 대여소 반납이 가능한 경우도 있습니다.',
  },
  {
    q: '초과 요금은 어떻게 발생하나요?',
    a: '기본 대여 시간 초과 시 추가 요금이 부과될 수 있습니다. 정확한 기준은 이용료 항목 또는 관리기관에 문의하세요.',
  },
  {
    q: '헬멧(안전모)은 어디서 구하나요?',
    a: '도로교통법에 따라 자전거 이용 시 안전모 착용을 강력 권장합니다. 일부 대여소에서 함께 대여해 드리며, 없는 경우 개인 헬멧을 지참하세요.',
  },
];
...
export default async function BikePage({ params }: PageProps) {
  ...
  const displayAddress = station.roadAddress || station.lotAddress || '주소 정보 없음';
  ...
  const naverUrl = hasCoords
    ...
    : `https://map.naver.com/v5/search/${encodeURIComponent(displayAddress)}`;

  const cards = [
    { icon: '🕐', label: '운영 시간',      value: station.operatingHours || '정보 없음', bg: '#eff6ff', border: '#bfdbfe' },
    { icon: '💰', label: '이용료',          value: station.feeDetails || station.feeType || '정보 없음', bg: '#f0fdf4', border: '#bbf7d0' },
    { icon: '🚲', label: '자전거 보유 대수', value: station.bikeCount ? `${station.bikeCount}대` : '정보 없음', bg: '#fdf4ff', border: '#e9d5ff' },
    { icon: '🅿️', label: '거치대 수',       value: station.rackCount ? `${station.rackCount}개` : '정보 없음', bg: '#fff7ed', border: '#fed7aa' },
    { icon: '💨', label: '공기주입기',       value: station.airPump || '정보 없음', bg: '#f0f9ff', border: '#bae6fd' },
    { icon: '🔧', label: '수리대',           value: station.repairBench || '정보 없음', bg: '#fefce8', border: '#fde68a' },
  ];
      ...
      >
        ...
        <div style={{ maxWidth: '880px', margin: '0 auto', position: 'relative' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '14px' }}>
            {sidoShort && (
              <span style={{ background: 'rgba(255,255,255,0.2)', backdropFilter: 'blur(8px)', borderRadius: '50px', padding: '4px 14px', fontSize: '0.8rem', fontWeight: 700 }}>
                {sidoShort}
              </span>
            )}
            {station.sigungu && (
            ...
            {station.eupmyeondong && (
              <span style={{ background: 'rgba(255,255,255,0.12)', borderRadius: '50px', padding: '4px 14px', fontSize: '0.8rem' }}>
                {station.eupmyeondong}
              </span>
            )}
          </div>
      ...
      <main style={{ maxWidth: '880px', margin: '0 auto', padding: '0 20px 72px' }}>
          ...
          >
            {cards.map(({ icon, label, value, bg, border }) => (
              ...
              >
                ...
                <div style={{ fontSize: '0.72rem', color: '#6b7280', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '4px' }}>
                  {label}
                </div>
                <div style={{ fontSize: '0.97rem', fontWeight: 700, color: '#111827', lineHeight: 1.4 }}>
                  {value}
                </div>
              </div>
            ))}
        ...
        >
          ...
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
            ...
            <AddressCopyButton address={displayAddress} />
          </div>
        ...
        <section aria-label="이용 안내" style={{ marginBottom: '36px' }}>
          ...
          >
            <ul style={{ margin: 0, paddingLeft: '20px', lineHeight: 2.2, color: '#374151', fontSize: '0.95rem' }}>
              <li><strong>반납 방법:</strong> 반납 후 거치대 잠금을 반드시 확인하고 앱/단말기에서 반납 완료 처리를 하세요.</li>
              ...
              <li><strong>출발 전 점검:</strong> 브레이크·타이어·라이트 등 안전 상태를 대여 전 반드시 확인하세요.</li>
